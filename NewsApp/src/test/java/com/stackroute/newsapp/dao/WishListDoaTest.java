...
public class WishListDoaTest {
    ...
    @Test
    public void addNewsToWishList() throws Exception {

        long newsID = 1234;
        when(newsDao.getNewsWithId(newsID)).thenReturn(news);
        given(wishListRepository.save(Mockito.any(WishList.class))).willReturn(wishList);
        String tempWishList = wishListDoa.addNewsToWishList(newsID, "userId");
    ...
    public void getAllWishListOfUser() throws Exception {
        ...
        List<WishList> tempWishList = wishListDoa.getAllWishListOfUser(userId);
